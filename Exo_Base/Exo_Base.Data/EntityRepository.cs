...
namespace Exo_Base.Data
{
    public class EntityRepository<TEntity> : IRepository<TEntity> where TEntity : BaseEntity
    {
        ...
        //public Task<List<TEntity>> GetAllAsync(Expression<Func<TEntity, int>> keySelector, OrderBy orderBy = OrderBy.Ascending)
        //{
        //    if (keySelector == null)
        //        keySelector = x => x.Id;
        //    return GetAllAsync(keySelector, null, orderBy);
        //}
        //public async Task<List<TEntity>> GetAllAsync(Expression<Func<TEntity, int>> keySelector, Expression<Func<TEntity, bool>> predicate, OrderBy orderBy, params Expression<Func<TEntity, object>>[] includeProperties)
        //{
        //    var entities = FilterQuery(keySelector, predicate, orderBy, includeProperties);
        ...
        {
            return _dbEntitySet.FirstOrDefaultAsync(x => x.Id == id);
        }
        public Task<TEntity> GetSingleIncludingAsync(int id, params Expression<Func<TEntity, object>>[] includeProperties)
        {
            var entites = IncludeProperties(includeProperties);
            return entites.FirstOrDefaultAsync(x => x.Id == id);
        }
        ...
        private IQueryable<TEntity> IncludeProperties(Expression<Func<TEntity, object>>[] includeProperties)
        {
